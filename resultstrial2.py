...
class Quiz:
    def __init__(self, partner, operation, question_list, correct_answers):
        ...
        self.check_button = Button(self.quiz_frame, text="Check", command=lambda: self.check(correct_answers))
        self.check_button.grid()

        self.next_button = Button(self.quiz_frame, text="Next", command=lambda: self.next(question_list, correct_answers), state=DISABLED)
    ...
    def next(self, question_list, correct_answers):
        ...
        if self.question_number == len(question_list):
            self.check_button.config(state=DISABLED)
            quiz_dict = {question:[answer, correct_answer] for question, answer, correct_answer in zip(question_list, self.answers, correct_answers)}
            for question, answer, correct_answer in quiz_dict:
                quiz_string += question, answer, correct_answer
            self.question_label.configure(text="Score: {} out of {}".format(self.correct_number, self.question_number))
            self.next_button.configure(text="Results", command=lambda: self.show_results(quiz_string))
