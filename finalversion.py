...
class Quiz:
    def __init__(self):
        ...
        self.main_menu_label = Label(self.menu_frame, text="Main Maths Menu", justify=CENTER, font="Helvetica 18", padx=10, pady=10)
        ...
        self.addition_button = Button(self.operations_frame, text="Addition +", command=lambda: self.generate_questions("Addition"), padx=10, pady=10, font="Helvetica 12", bg="cyan")
        ...
        self.subtraction_button = Button(self.operations_frame, text="Subtraction -", command=lambda: self.generate_questions("Subtraction"), padx=10, pady=10, font="Helvetica 12", bg="magenta")
        ...
        self.multiplication_button = Button(self.operations_frame, text="Multiplication x", command=lambda: self.generate_questions("Multiplication"), padx=10, pady=10, font="Helvetica 12", bg="orange")
        ...
        self.division_button = Button(self.operations_frame, text="Division /", command=lambda: self.generate_questions("Division"), padx=10, pady=10, font="Helvetica 12", bg="yellow")
        ...
        self.start_button = Button(self.quiz_frame, text="Start", command=lambda: self.display_question(), state=DISABLED, justify=CENTER, padx=5, pady=5, font="Helvetica 12", bg="white")
        self.start_button.grid(row=2, column=0)
        
        # Where all the questions show

        self.question_label = Label(self.quiz_frame, text="Questions will appear here.", justify=CENTER, font="Helvetica 12", padx=10, pady=10)
        ...
        self.check_button = Button(self.buttons_frame, text="Check", state=DISABLED, command=lambda: self.check(), padx=5, pady=5, font="Helvetica 12", bg="LightGoldenrod")
        self.check_button.grid(row=0, column=0)
        
        # Nex button to call the next function - determining what happens next in the program

        self.next_button = Button(self.buttons_frame, text="Next", state=DISABLED, command=lambda: self.next(), padx=5, pady=5, font="Helvetica 12", bg="LightGoldenrod1")
        self.next_button.grid(row=0, column=1)
        
        # The user can choose to quit at any time, enabling the four operation buttons again in doing so

        self.quit_button = Button(self.buttons_frame, text="Quit", state=DISABLED, command=self.close_quiz, padx=5, pady=5, font="Helvetica 12", bg="LightGoldenrod2")
        ...
        self.history_button = Button(self.buttons_frame, state=DISABLED, text="View History", command=lambda: self.show_results(self.quiz_history), padx=5, pady=5, font="Helvetica 12", bg="LightGoldenrod3")
        ...
        self.show_export_button = Button(self.export_frame, text="Export", justify=CENTER, state=DISABLED, command=lambda: self.export(self.quiz_history), padx=100, pady=5, font="Helvetica 12", bg="#9A9AFF")
        ...
        self.fun_zone_button = Button(self.export_frame, text="Fun Zone", justify=CENTER, state=DISABLED, command=self.fun, padx=100, pady=5, font="Helvetica 12", bg="gold")
        self.fun_zone_button.grid(row=1, column=1)
        
        # The help button, which opens the Help/Instructions window, open at all times

        self.help_button = Button(self.export_frame, text="Help", justify=CENTER, command=self.help, padx=100, pady=5, font="Helvetica 12", bg="silver")
...
class Results:
    def __init__(self, partner, quiz_history):
        ...
        self.back_button = Button(self.results_frame, text="Back", command=partial(self.close_results, partner), padx=5, pady=5, font="Helvetica 12", bg="white")
...
class Fun:
    def __init__(self, partner):
        ...
        self.back_button = Button(self.fun_frame, text="Back", command=partial(self.close_fun, partner), padx=5, pady=10, font="Helvetica 12", bg="white")
        self.back_button.grid()
...
class Help:
    def __init__(self, partner):
        ...
        self.help = Label(self.help_frame, font="Helvetica 10", text="Choose a quiz from the four operations.\nEverytime you score a 100%, you get access to Fun Zone, where you get a cool Maths Fact or Maths joke as a treat!\nYou can complete up to four quizzes in one session.\n", justify=CENTER)
        self.help.grid(row=2)

        self.back_button = Button(self.help_frame, text="Back", command=partial(self.close_help, partner), font="Helvetica 12", padx=5, pady=5, bg="white")
...
class Export:
    def __init__(self, partner, quiz_history):
        ...
        self.export = Label(self.export_frame, font="Helvetica 12", text="Save your quiz session history to a text file.", justify=LEFT, width=40, bg=background)
        self.export.grid(row=1)
        
        # Warning
        
        self.export_text = Label(self.export_frame, font="Helvetica 12", text="If the filename you enter below already exists, its content will be replaced with your quiz history from this session.", justify=LEFT, fg="maroon", padx=10, pady=10)
        ...
        self.save_button = Button(self.save_cancel_frame, padx=5, pady=5, font="Helvetica 12", text="Save", command=partial(lambda: self.save_quiz(partner, quiz_history)))
        self.save_button.grid(row=5, column=0)
        # The window closes should the user choose to click Cancel
        self.cancel_button = Button(self.save_cancel_frame, padx=5, pady=5, font="Helvetica 12", text="Cancel", command=partial(self.close_export, partner), bg="white")
