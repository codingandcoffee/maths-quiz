...
class Menu:
    def __init__(self):
        ...
        self.export_button = Button(text="Export", command=lambda: self.export(self.quiz_history), state=DISABLED)
...
class Quiz:
    def __init__(self, partner, operation, question_list, correct_answers):
        ...
        self.result_label = Label(self.quiz_frame, text="")
    ...
    def next(self, question_list, correct_answers):
        ...
        if self.question_number == len(question_list):
            ...
            self.next_button.configure(text="View Response", command=lambda: self.show_results(quiz_stri))
            self.next_button.config(state=NORMAL)
    ...
    def show_results(self, quiz_string, partner):
